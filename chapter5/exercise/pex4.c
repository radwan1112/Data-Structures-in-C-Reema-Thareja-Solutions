...
int concat_int(int day, int month, int year)
{
    char str[8];
...
void search_by_dob(struct student *ptr_stud)
{
    ...
    scanf(" %d", &year);
    getchar();
    int dob = concat_int(day, month, year);
    ...
    for(int i = 0; i < num_of_studs; i++)
    {
        if(concat_int(ptr_stud[i].birthday.day, ptr_stud[i].birthday.month, ptr_stud[i].birthday.year) == dob)
        {
            printf("%4d. | %11d | %10s %10s %10s | %2d/%2d/%4d | %3.2f (%c)\n", num + 1, ptr_stud[i].roll_no, ptr_stud[i].name.first_name, ptr_stud[i].name.middle_name, ptr_stud[i].name.last_name, ptr_stud[i].birthday.day, ptr_stud[i].birthday.month, ptr_stud[i].birthday.year, ptr_stud[i].average_grade, ptr_stud[i].overall_grade);
            key = ptr_stud[i].id;
            found = true;
...
int main()
{
    ...
    do
    {
        ...
        printf("3. Search for student record by date of birth.\n");
        printf("4. Show all Grades.\n");
        printf("5. Show all students with less than 40%% aggregate.\n");
        printf("6. Show all students with more than 90%% aggregate.\n");
        printf("7. Exit\n");
